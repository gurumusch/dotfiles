...
win_ct()                { bspc query -N -d -n .window.!floating | wc -l; }
...
echo "START (masterlayout) - ${master_area}" >> ~/foo.txt
case "$master_area" in
...
echo "START (masterlayout) - $(bspc query -N @/ -n .floating)"
while read a m d n _; do
	echo "START (masterlayout) - action = ${a}" >> ~/foo.txt
    if [[ "$a" = "node_remove" || "$a" = "node_transfer" ]] && was_master; then
        if [[ $(last_master_ct) -eq 1 && $(win_ct) -gt 1 ]] && ! is_last_master_aleaf; then
            ...
            bspc node biggest.window.local.!floating -n @/
        fi
	...
	echo "START (masterlayout) - lastmasters = ${lastmasters}" >> ~/foo.txt
